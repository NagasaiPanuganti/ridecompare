...
UBER_PRICE_RANGES: dict[RideCategory, tuple[float, float]] = {
    RideCategory.STANDARD: (12.0, 28.0),
    RideCategory.XL: (18.0, 42.0),
    RideCategory.PREMIUM: (35.0, 75.0),
}

UBER_ETA_RANGES: dict[RideCategory, tuple[int, int]] = {
    RideCategory.STANDARD: (3, 8),
    RideCategory.XL: (5, 12),
    RideCategory.PREMIUM: (7, 15),
}


class UberMockAdapter(ProviderAdapter):
    @property
    ...
    async def get_estimates(self, request: CompareRequest) -> list[EstimateResult]:
        try:
            ...
            if random.random() < 0.05:
                raise ConnectionError("Simulated Uber API timeout")

            category = request.category
            price_min, price_max = UBER_PRICE_RANGES[category]
            eta_min, eta_max = UBER_ETA_RANGES[category]
